...
namespace Wiki {
    ...
    void MergeInPlace(RandomAccessIterator first1, RandomAccessIterator last1,
        ...
        /*
         ...
         when none of the A or B blocks in any subarray contained 2sqrtA unique values,
         which places a hard limit on the number of times this will ACTUALLY need
         to binary search and rotate.

         according to my analysis the worst case is sqrtA rotations performed on sqrtA items
    ...
    void Sort(RandomAccessIterator first, RandomAccessIterator last, Comparison compare) {
        ...
        while (true) {
            ...
            if (iterator.length() < cache_size) {
                ...
                // 1. pull out two internal buffers each containing sqrtA unique values
                ...
                // we need to find either a single contiguous space containing 2sqrtA unique values (which will be split up into two buffers of size sqrtA each),
                // or we need to find one buffer of < 2sqrtA unique values, and a second buffer of sqrtA unique values,
                // OR if we couldn't find that many unique values, we need the largest possible buffer we can get

                // in the case where it couldn't find a single buffer of at least sqrtA unique values,
                ...
                while (!iterator.finished()) {
                    ...
                    if (count >= buffer_size) {
                        ...
                        if (count == buffer_size + buffer_size) {
                            // we were able to find a single contiguous section containing 2sqrtA unique values,
                            ...
                            // we found a buffer that contains at least sqrtA unique values, but did not contain the full 2sqrtA unique values,
                            // so we still need to find a second separate buffer of at least sqrtA unique values
                            ...
                            // we found a second buffer in an 'A' subarray containing sqrtA unique values, so we're done!
                    ...
                    if (count >= buffer_size) {
                        ...
                        if (count == buffer_size + buffer_size) {
                            ...
                            // we found a second buffer in a 'B' subarray containing sqrtA unique values, so we're done!
                ...
                while (!iterator.finished()) {
                    ...
                    if (start == pull[0].range.start) {
                        if (pull[0].from > pull[0].to) {
                            ...
                            // this only happens for very small subarrays, like sqrt4 = 2, 2 * (2 internal buffers) = 4,
                    ...
                    if (compare(*(B.end - 1), *A.start)) {
                        ...
                        for (RandomAccessIterator indexA = buffer1.start, indexB = firstA.end;
                             indexB < blockA.end;
                             ++indexA, indexB += block_size) {
                            std::iter_swap(indexA, indexB);
                        }
